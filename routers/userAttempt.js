...
router.post('/addattempt', async (req, res) => {
  ...
  var tup = await userattempt.findOneAndUpdate(
    { user: req.body.user },
    { $push: { attempt: req.body.attempt } },
    { upsert: true }
  );
  res.send(tup);
});
...
router.post('/getsubjectaccuracy', async (req, res) => {
  ...
  for (var i = 0; i < tup[0].attempt.length; i++) {
    if (tup[0].attempt[i].sub == sub) {
      tot++;
      if (tup[0].attempt[i].ac == true) correct++;
    }
  }
...
router.post('/getchapterpractice', async (req, res) => {
  ...
  const sub = req.body.sub;
  const chapter = req.body.chapter;
  console.log(grade, user, sub, chapter, 'rrr');

  const tup1 = await userattempt.find({ user: user });
  const tup2 = await question.find({
    grade: grade,
    sub: sub,
    chapter: chapter,
  });
  ...
  for (var i = 0; i < tup1[0].attempt.length; i++) {
    if (
      tup1[0].attempt[i].sub == sub &&
      tup1[0].attempt[i].chapter == chapter
    ) {
      tot++;
    }
  }
